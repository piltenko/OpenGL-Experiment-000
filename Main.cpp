...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "OpenGL test", nullptr, nullptr); // Windowed
	...
	Shader shader("resources/shaders/phong_lighting.vs", "resources/shaders/phong_lighting.fs");
	Shader lampShader("resources/shaders/light_source.vs", "resources/shaders/light_source.fs");

	Model ourModel("resources/models/assets/pallet/pallet.obj");
	ourModel.SetCoord(glm::vec3(-1.2, 0.0, 4.0));
	Model street_light_lamp("resources/models/street_light/street_light_lamp.obj");
	Model mug("resources/models/assets/test_level_002/test_level_002.obj");
	//Model lamp("resources/models/custom/monkey.obj");

	Model streetLightBody("resources/models/street_light/street_light.obj");
	Model streetLightLamp("resources/models/street_light/street_light_lamp.obj");
	streetLightBody.SetCoord(glm::vec3(-1.0, 8.0, 4.0));
	streetLightLamp.SetCoord(glm::vec3(-1.0, 8.0, 4.0));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//glUniform3f(glGetUniformLocation(shader.Program, "pointLights[0].position"), streetLight.GetCoord().x, streetLight.GetCoord().y, streetLight.GetCoord().z);
		...
		glUniform3f(glGetUniformLocation(shader.Program, "spotLight.position"), streetLightLamp.GetCoord().x, streetLightLamp.GetCoord().y, streetLightLamp.GetCoord().z);
		...
		glUniform1f(glGetUniformLocation(shader.Program, "spotLight.outerCutOff"), glm::cos(glm::radians(40.0f)));
		...
		model = glm::translate(model, streetLightBody.GetCoord());
		glUniformMatrix4fv(glGetUniformLocation(shader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
		streetLightBody.Draw(shader);

		//ourModel.SetCoord(glm::vec3(-4.0f + 3 * sin(glfwGetTime()), 2.0f, 7.0f + 3 * cos(glfwGetTime())));
		model = glm::mat4(1.0f);
		model = glm::translate(model, ourModel.GetCoord());
		glUniformMatrix4fv(glGetUniformLocation(shader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
		...
		//pointLightPositions[0].x = sin(0.2 * glfwGetTime());
		//pointLightPositions[0].z = cos(0.2 * glfwGetTime());
		...
		//street_light_lamp.Draw(lampShader);
		streetLightLamp.Draw(lampShader);
